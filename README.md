...
1. Run FluxCD bootstrap on the new cluster to overwrite the values in the `flux-system` directory in the cluster directory, this is required to complete the reconciliation loop between source and cluster.
    ...
    flux bootstrap gitlab --token-auth --owner "nominet/cyber/architecture-team" --repository "fluxcd-demo" --path "./clusters/cluster-a"
    ```
...
1. Bootstrap the clusters with FluxCD:
    ...
    flux bootstrap gitlab --context kind-staging --token-auth --owner nominet/cyber/architecture-team --repository fluxcd-demo --path ./clusters/staging 
